...
export function MonthView({ year: initialYear, month: initialMonth, bundle: initialBundle, stickyNotes: initialNotes }: Props) {
  ...
  const load = useCallback(async (y: number, m: number) => {
    ...
    const [b, notes] = await Promise.all([
      fetchRangeBundle(dateKey(start), dateKey(end)),
      fetchStickyNotesRange(dateKey(start), dateKey(end)),
    ]);
    setBundle(b);
  ...
  useEffect(() => {
    if (year !== initialYear || month !== initialMonth) load(year, month);
  }, [year, month, initialYear, initialMonth, load]);
  ...
  const monthPcts = Object.entries(pctMap).filter(([k]) => {
    const d = parseDateKey(k);
    return d.getFullYear() === year && d.getMonth() === month && k <= todayKey;
  });
  const avgPct = monthPcts.length
    ? Math.round(monthPcts.reduce((a, [, p]) => a + p, 0) / monthPcts.length)
    : 0;
  const perfectDays = monthPcts.filter(([, p]) => p === 100).length;
  const slackedDays = monthPcts.filter(([, p]) => p < 50).length;
    ...
    <div className="relative">
      ...
      <div className="flex items-end justify-between">
        <div>
          ...
          <p className="mt-1 text-[12px] text-black/40">{year}</p>
        </div>
